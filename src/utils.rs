...
pub fn add_to_etc_hosts(
    ...
    for line in reader.lines() {
        ...
        if line.contains(ip_address) && line.contains(hostname) {
            let output = imd::report(imd::IMDOutcome::Neutral, "Entry already in /etc/hosts, skipping");
...
pub fn create_dir(
    ...
    if fs::create_dir(ip_address).is_err() {
        let output = imd::report(imd::IMDOutcome::Neutral, "Directory already exists, skipping");
        bar.finish_with_message(format!("{starter_clone}{output}"));
        return Ok(());
    }
